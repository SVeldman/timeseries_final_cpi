...
```{r step_6}
# ============================================================================
# CROSS-VALIDATION FOR VAR INTERVENTION MODELS WITH LAG ORDER 2 AND LAG ORDER 3
# ============================================================================
# Testing lag 2 and lag 3 step intervention models
# 10-year sliding window, 1-month steps, 12-month forecast horizon
# ============================================================================
...
cat("Baseline VAR cross-validation completed successfully!\n")
cat("Training window:", k/12, "years | Forecast horizon:", H, "months | Iterations:", max_iterations, "\n")
cat("Mean AIC:", round(mean(aic_baseline), 2), "| Mean BIC:", round(mean(bic_baseline), 2), "\n")
cat("Mean Adj R² - CPI:", round(mean(adj_r2_baseline_cpi), 3), "| FedFunds:", round(mean(adj_r2_baseline_fedfunds), 3), "\n")
```
...
# MAE vs forecast horizon for both variables
...
# RMSE vs forecast horizon for both variables
...
# Legend for Horizon plots
plot.new()
legend("center", legend=c("CPI", "FedFunds"), col=c("blue", "red"), lwd=2, cex=1.2)

# AIC vs iteration number
...
# Adjusted R² vs iteration for both variables
...
# MAE by iteration (1-step ahead) for both variables
...
# RMSE by iteration (1-step ahead) for both variables
...
cat("Pre-intervention VAR cross-validation completed successfully!\n")
...
# Combined MAE vs forecast horizon for both variables
...
# Combined RMSE vs forecast horizon for both variables
...
# Combined AIC vs iteration number with transition line
...
# Combined BIC vs iteration number with transition line
...
# Combined Adjusted R² vs iteration for both variables with transition line
...
abline(v=transition_point, col="black", lty=2, lwd=2)
legend("topright", legend=c("CPI", "FedFunds"), col=c("blue", "red"), lwd=2)
text(transition_point + 5, max(combined_adj_r2_cpi, na.rm=TRUE) * 0.95, "Baseline → Intervention", adj=0, cex=0.8)

# Combined MAE by iteration (1-step ahead) for both variables with transition line
...
# Combined RMSE by iteration (1-step ahead) for both variables with transition line
...
text(0.05, 0.7, paste("Iterations:", max_pre_iterations), adj=0, cex=0.9)
text(0.05, 0.65, paste("Mean AIC:", round(mean(aic_pre), 2)), adj=0, cex=0.9)
text(0.05, 0.6, paste("Mean BIC:", round(mean(bic_pre), 2)), adj=0, cex=0.9)
text(0.05, 0.55, paste("Adj R² (CPI):", round(mean(adj_r2_pre_cpi), 3)), adj=0, cex=0.9)
text(0.05, 0.5, paste("Adj R² (FedFunds):", round(mean(adj_r2_pre_fedfunds), 3)), adj=0, cex=0.9)

# Intervention summary
text(0.75, 0.8, "Intervention Period", font=2, cex=1, col="red")
text(0.55, 0.7, paste("Iterations:", intervention_iterations), adj=0, cex=0.9)
text(0.55, 0.65, paste("Mean AIC:", round(mean(aic_intervention), 2)), adj=0, cex=0.9)
text(0.55, 0.6, paste("Mean BIC:", round(mean(bic_intervention), 2)), adj=0, cex=0.9)
text(0.55, 0.55, paste("Adj R² (CPI):", round(mean(adj_r2_intervention_cpi), 3)), adj=0, cex=0.9)
text(0.55, 0.5, paste("Adj R² (FedFunds):", round(mean(adj_r2_intervention_fedfunds), 3)), adj=0, cex=0.9)

# Performance differences
text(0.5, 0.4, "Performance Differences", font=2, cex=1)
aic_diff <- mean(aic_intervention) - mean(aic_pre)
bic_diff <- mean(bic_intervention) - mean(bic_pre)
r2_cpi_diff <- mean(adj_r2_intervention_cpi) - mean(adj_r2_pre_cpi)
r2_fedfunds_diff <- mean(adj_r2_intervention_fedfunds) - mean(adj_r2_pre_fedfunds)

text(0.1, 0.3, paste("ΔAIC:", round(aic_diff, 2), ifelse(aic_diff > 0, "(worse)", "(better)")), adj=0, cex=0.9)
text(0.1, 0.25, paste("ΔBIC:", round(bic_diff, 2), ifelse(bic_diff > 0, "(worse)", "(better)")), adj=0, cex=0.9)
text(0.1, 0.2, paste("ΔAdj R² (CPI):", round(r2_cpi_diff, 3), ifelse(r2_cpi_diff > 0, "(better)", "(worse)")), adj=0, cex=0.9)
text(0.1, 0.15, paste("ΔAdj R² (FedFunds):", round(r2_fedfunds_diff, 3), ifelse(r2_fedfunds_diff > 0, "(better)", "(worse)")), adj=0, cex=0.9)

text(0.5, 0.05, paste("Total Periods Analyzed:", total_iterations, "| Transition at:", round(transition_point)), adj=0.5, cex=0.9)
```
